...
type Diaria struct {
	...
	HumitatRelativa int `json: "humetatRelativa"`
	Time time.Time `json:"-"`
}
...
func GetPrediccio(url, string) (*Diaria, error) {
	...
	//control d' error
	if err != nil {
		log.Println("Error leyendo Json")
		return nil, err //Preparamos los retornos
	}
	...
	tempMin = prediccio[0].Prediccion.Dia[0].Temperatura.Maxima
	humitat = prediccio[0].Prediccion.Dia[0].HumetatRelativa.Maxima

	var infoActual := Diaria{
